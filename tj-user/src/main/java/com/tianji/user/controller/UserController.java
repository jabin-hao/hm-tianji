...
public class UserController {
    ...
    public void resetPassword(
            @Parameter(description = "要重置的用户的 id", example = "1") @PathVariable("id") Long userId) {
    ...
    public void updateUserStatus(
            @Parameter(description = "要重置的用户的 id", example = "1") @PathVariable("id") Long userId,
            @PathVariable @Parameter(description = "状态", example = "1") Integer status
    ) {
        User user = new User();
        user.setId(userId);
    ...
    public LoginUserDTO queryUserDetail(
            @Valid @RequestBody LoginFormDTO loginDTO, @PathVariable boolean isStaff) {
        return userService.queryUserDetail(loginDTO, isStaff);
    }

    /**
     * <h1>根据 id 批量查询用户信息</h1>
     *
     * @param ids 用户 id 集 合
     * @return 用户集合
     */
    ...
    public List<UserDTO> queryUserByIds(
            @Parameter(description = "用户 id 的列表") @RequestParam("ids") List<Long> ids) {
    ...
    @Hidden
    @GetMapping("/{id}/type")
    public Integer queryUserType(@PathVariable Long id) {
