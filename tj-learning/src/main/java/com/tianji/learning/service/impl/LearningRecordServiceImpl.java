...
public class LearningRecordServiceImpl extends ServiceImpl<LearningRecordMapper, LearningRecord> implements ILearningRecordService {
    ...
    public void addLearningRecord(LearningRecordFormDTO recordDTO) {
        ...
        ((ILearningRecordService) AopContext.currentProxy()).handleLearningLessonsChanges(recordDTO);
    }
    ...
    public void handleLearningLessonsChanges(LearningRecordFormDTO recordDTO) {
        ...
        //判断是否学完所有小节
        boolean allLearned = lesson.getLearnedSections() + 1 >= courseFullInfoDTO.getSectionNum();
        //3.更新课表
...
//        boolean finished = !one.getFinished() && recordDTO.getDuration() <= recordDTO.getMoment() * 2;
//        //2.2更新小节学习，如果符合完成条件，则更新小节记录的完成时间和状态
//        updateLearningRecord(recordDTO, one, finished);
//        return finished;
//    }
    ...
    private boolean handleVideoRecordWithRedis(Long userId, LearningRecordFormDTO recordDTO) {
        ...
        //2.有学习记录，修改学习记录
        //2.1如果该小节视频长度完成50%及以上且小节旧的状态为未完成，才让当前课程表的小节完成数量+1
        boolean finished = !oldRecord.getFinished() && recordDTO.getDuration() <= recordDTO.getMoment() * 2;
        ...
        //2.2更新小节学习，如果符合完成条件，则更新小节记录的完成时间和状态
        updateLearningRecord(recordDTO, oldRecord);
        //3.如果是第一次学完，则清除redis中的学习记录
        taskHandler.cleanRecordCache(recordDTO.getLessonId(), recordDTO.getSectionId());
        return true;
    }

    private void updateLearningRecord(LearningRecordFormDTO recordDTO, LearningRecord oldRecord) {
        boolean update = lambdaUpdate()
                .set(LearningRecord::getMoment, recordDTO.getMoment())
                .set(true, LearningRecord::getFinished, true)
                .set(true, LearningRecord::getFinishTime, recordDTO.getCommitTime())
