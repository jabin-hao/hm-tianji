...
public class InteractionQuestionServiceImpl extends ServiceImpl<InteractionQuestionMapper, InteractionQuestion> implements IInteractionQuestionService {
    ...
    public PageDTO<QuestionAdminVO> queryQuestionPageAdmin(QuestionAdminPageQuery query) {
        ...
        //3.远程批量查询章节信息
        Map<Long, String> cataMap = seekCatalogueInfo(chapterAndSectionIds);
        //4.远程批量查询用户信息
        List<UserDTO> userDTOS = userClient.queryUserByIds(uIds);
        if(CollUtils.isEmpty(userDTOS)){
            throw new BizIllegalException("用户集合不存在");
        }
        Map<Long, String> userMap = UserUtils.toUserNameMap(userDTOS);
        //6.封装vo
    ...
    private Map<Long, String> seekCatalogueInfo(ArrayList<Long> chapterAndSectionIds) {
        ...
        return catas.stream().collect(Collectors.toMap(CataSimpleInfoDTO::getId, CataSimpleInfoDTO::getName));
    }
    ...
    public QuestionAdminVO queryQuestionByIdAdmin(Long id) {
        ...
        vo.setTeacherName(userMap.get(course.getTeacherIds().getFirst()).getName());
        //6.3设置章节信息
        ...
        vo.setCategoryName(categoryNames);
        //6.5设置被回答数量
        Long answerTimes = replyService.lambdaQuery()
                .eq(InteractionReply::getQuestionId, id)
                .eq(InteractionReply::getAnswerId, 0L)
                .count();
        vo.setAnswerTimes(Math.toIntExact(answerTimes));
        //7.修改问题状态为已查看
