...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements ICategoryService {
    ...
    public List<CategoryVO> list(CategoryListDTO categoryListDTO) {
        ...
        if (CollUtils.isNotEmpty(categoryVOS)) {
            return filter(categoryVOS, categoryListDTO);
        } else {
            return new ArrayList<>();
        }
    }
    ...
    public void delete(Long id) {
        ...
        //3.统计分类拥有的课程数量
        Long courseNum = courseService.countCourseNumOfCategory(id);
        //3.1.分类拥有课程数据量判空
        if (courseNum > 0) {
        ...
        if (result <= 0) {
            throw new DbException(CourseErrorInfo.Msg.CATEGORY_DELETE_FAILED);
        }
    }
    ...
    public void disableOrEnable(CategoryDisableOrEnableDTO categoryDisableOrEnableDTO) {
        ...
        if (CollUtils.isNotEmpty(categories)) { //直接子分类
            ...
            childCategoryIds.addAll(categories.stream().map(Category::getId).toList());
        }
        //3.3获取启用分类子分类的子分类列表
        if (CollUtils.isNotEmpty(childCategoryIds)) {
            ...
            List<Category> inDirectCategoryList = baseMapper.selectList(inDirectQueryWrapper);
            if (CollUtils.isNotEmpty(inDirectCategoryList)) {
                //3.4将子分类的子分类id列表写入childCategoryIds中
                childCategoryIds.addAll(inDirectCategoryList.stream()
        ...
        if (categoryDisableOrEnableDTO.getStatus() == CommonStatus.DISABLE.getValue()) {
            ...
            taskExecutor.execute(() -> batchDownShelfCourse(category.getId(), category.getLevel(), userId));
    ...
    public void update(CategoryUpdateDTO categoryUpdateDTO) {
        ...
        //3.设置更新字段
        Category updateCategory = new Category();
        updateCategory.setId(categoryUpdateDTO.getId()); //修改课程分类 id
    ...
    public List<CategoryVO> allOfOneLevel() {
        ...
        Map<Long, Long> thirdCategoryNumMap = this.statisticThirdCategory();
        return BeanUtils.copyList(list, CategoryVO.class, (category, categoryVO) -> categoryVO.setThirdCategoryNum(thirdCategoryNumMap.getOrDefault(category.getId(), 0L).intValue()));
    }
    ...
    private void checkSameName(Long parentId, String name, Long currentId) {
        ...
        if (currentId != null && CollectionUtil.isNotEmpty(categories) && categories.getFirst().getId() != currentId.longValue()) {
            throw new BizIllegalException(CourseErrorInfo.Msg.CATEGORY_SAME_NAME);
        }
    }
     ...
     * 根据条件过滤课程分类
     */
    private List<CategoryVO> filter(List<CategoryVO> categoryVOList, CategoryListDTO categoryListDTO) {
        if (CollUtils.isEmpty(categoryVOList)) {
            return new ArrayList<>();
        }
    ...
    private boolean filter(CategoryVO categoryVO, CategoryListDTO categoryListDTO) {
        ...
        boolean pass = true;
        // 状态校验
        if (categoryListDTO.getStatus() != null) { //和查询状态一致 pass
            pass = (Objects.equals(categoryVO.getStatus(), categoryListDTO.getStatus()));
        }
        //名称校验
