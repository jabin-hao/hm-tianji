...
public class CourseCatalogueDraftServiceImpl extends ServiceImpl<CourseCatalogueDraftMapper, CourseCatalogueDraft> implements ICourseCatalogueDraftService {
    ...
    private final  CourseCataSubjectMapper courseCataSubjectMapper;
    ...
    public void save(Long courseId, List<CataSaveDTO> cataSaveDTOS, Integer step) {
        ...
        if (cataSaveDTOS.size() != cataSaveDTOS
                ...
                .distinct()
                .count()) {
            throw new BizIllegalException(CourseErrorInfo.Msg.COURSE_CATAS_SAVE_INDEX);
        }
        if (cataSaveDTOS.size() < cataSaveDTOS.getLast().getIndex()) {
            throw new BizIllegalException(CourseErrorInfo.Msg.COURSE_CATAS_SAVE_INDEX_JUMP);
        }
        ...
        if (step == CourseConstants.CourseStep.CATALOGUE) {
            ...
            courseCatalogueDraftMapper.deleteByCourseId(courseId,
                    Arrays.asList(
                            CourseConstants.CataType.CHAPTER,
                            CourseConstants.CataType.SECTION,
                            CourseConstants.CataType.PRACTICE));
        } else {
    ...
    public List<CataVO> queryCourseCatalogues(Long courseId, Boolean see, Boolean withPractice) {
        ...
        if (see) {
            //1.1查询正式数据目录
            cataVOS = courseCatalogueService.queryCourseCataloguesVO(courseId, withPractice);
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {DbException.class, Exception.class})
    public void saveSubject(Long courseId, List<CataSubjectDTO> cataSubjectDTOS) {
        //1.数据校验
        //1.1.转化目录id列表
        ...
        for (CataSubjectDTO cataSubjectDTO : cataSubjectDTOS) {
            for (Long subjectId : cataSubjectDTO.getSubjectIds()) {
                CourseCataSubjectDraft courseCataSubjectDraft = getCourseCataSubjectDraft(courseId, cataSubjectDTO, subjectId);
                //3.4.课程题目关系添加到
    ...
    private static @NonNull CourseCataSubjectDraft getCourseCataSubjectDraft(Long courseId, CataSubjectDTO cataSubjectDTO, Long subjectId) {
        ...
        return courseCataSubjectDraft;
    }

    @Override
    public List<CataSimpleSubjectVO> getSubject(Long courseId) {
    ...
    public void checkCataInfoImplated(Long courseId) {
        ...
        //校验练习或小节有没有上传题目和媒资
        CollUtils.check(courseCatalogueDrafts, new Checker<>() {
            @Override
            public void check(CourseCatalogueDraft courseCatalogueDraft) {
                ...
                } else if (courseCatalogueDraft.getType() == CourseConstants.CataType.PRACTICE
    ...
    @Override
    public Long totalSectionNums(Long courseId) {
        LambdaQueryWrapper<CourseCatalogueDraft> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(CourseCatalogueDraft::getCourseId, courseId)
                .in(CourseCatalogueDraft::getType,
                        Arrays.asList(CourseConstants.CataType.SECTION, CourseConstants.CataType.PRACTICE));
    ...
    private void checkIndex(List<CataSaveDTO> cataSaveDTOS, List<CourseCatalogue> courseCatalogues) {
        ...
        if (cataSaveDTOS.getLast().getIndex() > cataSaveDTOS.size()) {
            ...
            cataSaveDTO.getSections().stream().filter(section -> section.getType() == CourseConstants.CataType.SECTION)
                    .forEach(section -> {
                        if (section.getId() == null) {
                            //序号从1开始
                            saveIndexMap.put(null, count.incrementAndGet());
                        }
                    });
        }
    ...
    private List<CourseCatalogueDraft> packageCatalogue(Long courseId, List<CataSaveDTO> cataSaveDTOS, List<CourseCatalogue> shelfCourseCatalogues) {
        ...
        if (CollUtils.isNotEmpty(shelfCourseCatalogues)) {
            //将已经上架的目录 copy下来
            shelfCourseCatalogues.forEach(courseCatalogue ->
        ...
        if (CollUtils.isNotEmpty(savedCourseCataloguesDraft)) {
            //存在草稿覆盖已经上架的课程
            savedCourseCataloguesDraft.      forEach(courseCatalogueDraft ->
        ...
        for (CataSaveDTO cataSaveDTO : cataSaveDTOS) {
            ...
            cataSaveDTO.getSections().forEach(section -> {
    ...
    private List<CataVO> queryCourseCatalogues(Long courseId, Boolean withPractice) {
        ...
        Map<Long, CourseCatalogueDraft> chapterIdAndMaxSectionMap =
                (Objects.equals(courseDraft.getStatus(), CourseStatus.NO_UP_SHELF.getStatus()))
                        ...
                        courseCatalogueDrafts.parallelStream()
                                ...
                                .collect(Collectors.toMap(CourseCatalogueDraft::getParentCatalogueId, Function.identity(), (c1, c2) -> c2.getCIndex().compareTo(c1.getCIndex()) > 0 ? c2 : c1));
        ...
        return TreeDataUtils.parseToTree(courseCatalogueDrafts, CataVO.class, (catalogueDraft, vo) -> {
            ...
            if (catalogueDraft.getType() == CourseConstants.CataType.SECTION) {
                ...
                maxIndexOnShelf = courseCatalogueDraft == null ? 0 : courseCatalogueDraft.getCIndex();
     ...
     *
     * @param cataIds  前端传过来的小节 id列表或者练习列表
     * @param courseId 课程 id
     */
    private void checkPracticeIds(List<Long> cataIds, Long courseId) {
        ...
        //所有练习的 id列表
        List<Long> practiceIdList = courseCatalogueDrafts.stream().filter(practice -> practice.getType() == CourseConstants.CataType.PRACTICE).
                map(CourseCatalogueDraft::getId).collect(Collectors.toList());
        //判断前端传过来的章节 id列表是否符合是课程练习id父集合
     ...
     * 课程目录草稿树型数据转换器
     */
    private static class CourseCatalogDraftDataWrapper implements TreeDataUtils.DataProcessor<CataVO, CourseCatalogueDraft> {
