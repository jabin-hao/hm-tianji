...
import com.tianji.common.utils.CollectionUtils;
import com.tianji.common.utils.CounterUtils;
import com.tianji.api.utils.CourseUtils;
...
public class CourseDraftServiceImpl extends ServiceImpl<CourseDraftMapper, CourseDraft> implements ICourseDraftService {
    ...
    public CourseBaseInfoVO getCourseBaseInfo(Long id, Boolean see) {
        ...
        if (see) {
            ...
            if (course != null) {
                ...
                //1.4.设置课程评分
                courseBaseInfoVO.setCourseScore(NumberUtils.div(NumberUtils.null2Zero(course.getScore()) * 1.0, 10, 2));
                //1.5.设置报名人数
        ...
        if (courseBaseInfoVO == null) {
            ...
            if (courseDraft != null) {
                ...
                //2.9.设置课程评分
                courseBaseInfoVO.setCourseScore(0d);
                //2.10.设置报名人数
        ...
        List<UserDTO> userDTOS = userClient.queryUserByIds(
                Stream.of(courseBaseInfoVO.getCreater(), courseBaseInfoVO.getUpdater())
                        .distinct()
                        .collect(Collectors.toList())
        );
        if (CollUtils.isNotEmpty(userDTOS)) {
            //3.1.创建者，更新至id+name映射关系
            Map<Long, String> operatorMap = UserUtils.toUserNameMap(userDTOS);
            //3.2.设置创建者姓名
        ...
        if (CollUtils.isNotEmpty(categories)) {
            //4.1分类id和名称关系
            Map<Long, String> categoryIdAndName = CollectionUtils.toMap(categories, Category::getId, Category::getName);
            //4.2.设置课程分类名称
    ...
    public void updateStep(Long id, Integer step) {
        ...
        if (CourseConstants.CourseStep.CATALOGUE == step ||
            ...
            updateCourseDraft.setSectionNum(Math.toIntExact(courseCatalogueDraftService.totalSectionNums(id)));
        }
        //4.更新課程狀態
    ...
    public void upShelf(Long id) {
        ...
        if (isFirstUpShelf) {
            ...
            //4.6.再次上架
            //4.6.1.更新正式课程内容信息
            result = courseContentMapper.updateById(courseContent);
            if (result <= 0) {
                throw new DbException(ErrorInfo.Msg.DB_UPDATE_EXCEPTION);
            }
            //4.6.2.更新正式课程基本信息
    ...
    public void checkBeforeUpShelf(Long id) {
        ...
        if (courseDraft == null && course != null) {
            throw new BizIllegalException(CourseErrorInfo.Msg.COURSE_UP_SHELF_ALREADY);
        }
        //2.2.课程id不存在的课程无法上架
        if (courseDraft == null) {
        ...
        //课程
        //2.4.已上架/已完结课程不能上架
        if (course != null && !Objects.equals(course.getStatus(), CourseStatus.DOWN_SHELF.getStatus())) {
    ...
    public void downShelf(Long id) {
        ...
        if (course == null || !course.getStatus().equals(CourseStatus.SHELF.getStatus())) {
            throw new BizIllegalException(CourseErrorInfo.Msg.COURSE_DOWN_SHELF_FAILED);
        }
        //2.先更新课程状态
        Course update = new Course();
        update.setId(id);
        update.setStatus(CourseStatus.DOWN_SHELF.getStatus());
        update.setUpdateTime(DateUtils.now());
        int upd = courseMapper.updateById(update);
        if (upd != 1) {
            throw new DbException(ErrorInfo.Msg.DB_UPDATE_EXCEPTION);
        }
    ...
    public PageDTO<CoursePageVO> queryForPage(CoursePageQuery coursePageQuery) {
        ...
        //2.组装数据查询
        //2.1.课程更新人id列表
        List<Long> updaterList = CollectionUtils.extractToList(page.getRecords(), CourseDraft::getUpdater);
        //2.2.查询更新人用户信息
        List<UserDTO> userDTOS = userClient.queryUserByIds(updaterList);
        //2.3.转化更新人用户id+name 映射关系
        Map<Long, String> updaterMap = UserUtils.toUserNameMap(userDTOS);
        //2.4.查询课程分类列表
        List<Category> list = categoryService.list();
        //2.5.转化课程分类id+name映射关系
        Map<Long, String> categoryNameMap = CollectionUtils.toMap(list, Category::getId, Category::getName);
        //2.6.课程id列表
        List<Long> courseIdList = CollectionUtils.extractToList(page.getRecords(), CourseDraft::getId);
        //2.7.统计课程报名人数map
        Map<Long, Integer> peoNumOfCourseMap = CourseUtils.getEnrollmentCountMap(tradeClient, courseIdList);
        //3.数据封装
    ...
    public List<Long> queryExists(List<Long> idList) {
        ...
        //2.组装数据
        return CollectionUtils.extractToList(courses, CourseDraft::getId);
    }
    ...
    public Map<Long, Integer> countCourseNumOfCategory() {
        ...
        //2.统计每个课程分类拥有的课程数量
        return CounterUtils.countByMultiple(courses, 
                CourseDraft::getFirstCateId,
                CourseDraft::getSecondCateId, 
                CourseDraft::getThirdCateId);
    }
