...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements ICourseService {
    ...
    public CourseDTO getCourseDTOById(Long id) {
        ...
        if (CollUtils.isNotEmpty(courseTeachers)) {
            courseDTO.setTeacher(courseTeachers.getFirst().getTeacherId());
        } else {
            courseDTO.setTeacher(0L);
        }

        // 4.统计课程销量
        Integer soldCount = CourseUtils.getSingleCourseEnrollment(tradeClient, id);
        courseDTO.setSold(soldCount);
        //5.返回数据
        return courseDTO;
    ...
    public CourseAndSectionVO queryCourseAndCatalogById(Long courseId) {
        ...
        if (CollUtils.isNotEmpty(teachers)) {
            UserDTO teacher = teachers.getFirst();
            vo.setTeacherName(teacher.getName());
            vo.setTeacherIcon(teacher.getIcon());
        }
        // 5.组装小节信息
        List<CatalogueDTO> catalogueList = course.getChapters();
        List<ChapterVO> chapters = new ArrayList<>(catalogueList.size());
        for (CatalogueDTO c : catalogueList) {
        ...
        if(latestSectionId == null) {
            latestSectionId = records.getFirst().getSectionId();
        }
    ...
    @Override
    @Cacheable(cacheNames = RedisConstants.Formatter.STATISTICS_COURSE_NUM_CATE)
    public Map<Long, Integer> countCourseNumOfCategory() {
        //1.统计课程分类拥有已上架、已完成的课程数量
        Map<Long, Integer> normalCourseNumOfCategory =
                countNormalCourseNumOfCategory();
        //2.统计课程分类待上架，已下架的课程分类的数量
        Map<Long, Integer> draftCourseNumOfCategory =
                courseDraftService.countCourseNumOfCategory();
        //3.两组数据聚合
        return CollUtils.union(normalCourseNumOfCategory, draftCourseNumOfCategory);
    }
    ...
    public List<Long> getCategoryIdListWithCourse() {
        ...
        category3s.forEach(category3-> category3.setId(categoryIdList));
        // 2.1.去重，并返回数据
    ...
    @Override
    public Long countCourseNumOfCategory(Long categoryId) {
        //1.课程统计条件
        LambdaQueryWrapper<Course> queryWrapper =
                Wrappers.lambdaQuery(Course.class)
    ...
    public PageDTO<CoursePageVO> queryForPage(CoursePageQuery coursePageQuery) {
        ...
        //2.课程更新人id列表
        List<Long> updaterList = CollectionUtils.extractToList(page.getRecords(), Course::getUpdater);
        //2.1查询更新人用户信息
        ...
        //4.课程id列表
        List<Long> courseIdList = CollectionUtils.extractToList(page.getRecords(), Course::getId);
        //4.1.统计每个课程报名人数map关系
     ...
     * 统计课程分类上架和已完结课程的数量
     */
    private Map<Long, Integer> countNormalCourseNumOfCategory() {
        //1.查询条件
        ...
        for (Course course : courses) {
            //3.1一级分类数量
            cateIdAndNumMap.compute(course.getFirstCateId(), (k, firstCateNum) -> firstCateNum == null ? 1 : firstCateNum + 1);
            //3.2二级分类数量
            cateIdAndNumMap.compute(course.getSecondCateId(), (k, secondCateNum) -> secondCateNum == null ? 1 : secondCateNum + 1);
            //3.3三级分类数量够
            cateIdAndNumMap.compute(course.getThirdCateId(), (k, thirdCateNum) -> thirdCateNum == null ? 1 : thirdCateNum + 1);
        }
