...
public class RoleController {
    ...
    @Operation(description ="查询员工角色列表")
    @GetMapping("/list")
    ...
    @Operation(description ="根据 id查询角色")
    @GetMapping("/{id}")
    public RoleDTO queryRoleById(@PathVariable Long id){
        // 1.查询
    ...
    public void updateRole(
            ...
            @PathVariable @Parameter(name = "角色 id", example = "1") Long id
    ) {
        // 1.数据转换
        Role role = new Role(roleDTO);
        role.setId(id);
    ...
    @DeleteMapping("{id}")
    public void deleteRole(@PathVariable @Parameter(name = "角色 id", example = "1") Long id) {
