...
public class PrivilegeServiceImpl extends ServiceImpl<PrivilegeMapper, Privilege> implements IPrivilegeService {
    ...
    public List<PrivilegeRoleDTO> listPrivilegeRoles() {
        ...
        for (Privilege p : privileges) {
            // 4.1.根据权限查询角色，处理可能的null值
            List<RolePrivilege> rolePrivileges = rpMap.get(p.getId());
            Set<Long> roles = CollectionUtil.isEmpty(rolePrivileges) ? 
    ...
    public void bindRolePrivileges(Long roleId, List<Long> privilegeIds) {
        ...
        // 2.判断权限是否存在
        validatePrivilegesExist(privilegeIds);
        // 3.绑定关系
        List<RolePrivilege> rolePrivileges = privilegeIds.stream()
                .map(privilegeId -> new RolePrivilege(roleId, privilegeId))
     ...
     * 验证权限是否存在
     */
    private void validatePrivilegesExist(List<Long> privilegeIds) {
        Long privilegeCount = lambdaQuery().in(Privilege::getId, privilegeIds).count();
        if (privilegeCount == null || !privilegeCount.equals((long) privilegeIds.size())) {
