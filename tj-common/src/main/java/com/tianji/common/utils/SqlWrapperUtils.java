...
public class SqlWrapperUtils {
     ...
     * 用于生成查询语句 columns
     */
    public static String getSqlColumns(Class<?> clazz, String prefix) {
     ...
     *
     * @param queryDTO    sql 查询条件 dto
     * @param targetClazz 查询的数据库对应的 po 的 class
     * @param <T>         查询的数据库对应的 po 的类型
     * @param <R>         需要转换的条件对象
     ...
     *
     * @param queryWrapper 查询 QueryWrapper
     * @param prefix 查询条件前缀
     * @param queryDTO 查询实体
     ...
     * @param <R> 查询实体类型
     */
    public static <T, R> void suppleQueryWrapper(QueryWrapper<?> queryWrapper, String prefix, R queryDTO, Class<T> targetClass) {
        //将查询 dto 转换成目标对象
        T target = BeanUtils.toBean(queryDTO, targetClass);
        //获取目标对象的字段
        ...
        if (ArrayUtils.isNotEmpty(fields)) {
            ...
            for (Field field : fields) {
                ...
                if (value != null && ReflectUtils.getMethod(targetClass, "get" + StringUtils.upperFirst(field.getName())) != null) {
     ...
     *
     * @param updateDTO   sql 更新条件 dto
     * @param targetClazz 更新的数据库对应的 po 的 class
     * @param <T>         更新的数据库对应的 po 的类型
     * @param <R>         需要转换的条件对象
     * @return LambdaQueryWrapper 对象
     */
