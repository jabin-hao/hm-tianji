...
import com.tianji.common.utils.FileUtils;
import com.tianji.common.utils.RetryUtils;
...
public class TencentMediaStorage implements IMediaStorage {
    ...
    private CommitUploadResponse commitUpload(ApplyUploadResponse applyUploadResponse) {
        ...
        return RetryUtils.executeWithRetry(
                () -> vodClient.CommitUpload(request),
                4,
                (TencentCloudSDKException e) -> StringUtils.isBlank(e.getRequestId()),
                MEDIA_COMMIT_UPLOAD_ERROR
        );
    }
    ...
    private ApplyUploadResponse applyUpload(String filename) {
        ...
        req.setMediaType(FileUtils.getFileExtension(filename));
        req.setProcedure(tencentProperties.getVod().getProcedure());
        
        return RetryUtils.executeWithRetry(
                () -> vodClient.ApplyUpload(req),
                4,
                (TencentCloudSDKException e) -> StringUtils.isBlank(e.getRequestId()),
                MEDIA_APPLY_UPLOAD_ERROR
        );
    }
    ...
    public List<Media> queryMediaInfos(String ... fileIds) {
        ...
        try {
            ...
            throw new CommonException("获取媒资信息异常", e);
        }
