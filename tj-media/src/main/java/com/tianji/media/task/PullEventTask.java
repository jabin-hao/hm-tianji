...
public class PullEventTask {
    ...
    public void pullEvent() {
        ...
        try {
            ...
            if(e.getMessage().equals("no event")){
                log.debug("暂无 event事件");
            }else{
                log.error("VOD 事件处理异常", e);
            }
        }
    }
    ...
    private void handleProcedureStateChangeEvent(EventContent ec) {
        ...
        if (PROCEDURE_EVENT_FINISH.equals(pt.getStatus())) {
            ...
            mediaService.updateMediaProcedureResult(getMedia(optional, pt, md));
        }
    }

    private static @NonNull Media getMedia(Optional<MediaProcessTaskResult> optional, ProcedureTask pt, MediaMetaData md) {
